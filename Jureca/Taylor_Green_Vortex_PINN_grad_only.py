...
def train(model, device , train_loader, optimizer, epoch,grank, gwsize, rho, nu, p_max, p_min):
    model.train()
    t_list = []
    loss_acc = 0
    if grank==0:
    ...
    for batch_idx, (data) in enumerate(train_loader):
        ...
        loss = total_loss(model, data, device, rho, nu, p_max, p_min, grank)
...
def total_loss(model, data, device, rho, nu, p_min, p_max, grank):
    ...
    u_x_y_t = torch.autograd.grad(u, g, torch.ones(inputs.shape[0], 1).to(device), retain_graph = True, create_graph = True)[0]
    v_x_y_t = torch.autograd.grad(v, g, torch.ones(inputs.shape[0], 1).to(device), retain_graph = True, create_graph = True)[0]
    p_x_y_t = torch.autograd.grad(p, g, torch.ones(inputs.shape[0], 1).to(device), retain_graph = True, create_graph = True)[0]
    ...
    ux2 = torch.autograd.grad(ux, g, torch.ones(inputs.shape[0],1).to(device), create_graph = True)[0]
    uy2 = torch.autograd.grad(uy, g, torch.ones(inputs.shape[0],1).to(device), create_graph = True)[0]

    vx2 = torch.autograd.grad(vx, g, torch.ones(inputs.shape[0],1).to(device), create_graph = True)[0]
    vy2 = torch.autograd.grad(vy, g, torch.ones(inputs.shape[0],1).to(device), create_graph = True)[0]
    ...
    loss_variable = loss_function(output, exact)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if args.benchrun and epoch==args.epochs:
            with torch.autograd.profiler.profile(use_cuda=args.cuda, profile_memory=True) as prof:
                loss_acc = train(distrib_model, device, train_loader, optimizer,epoch, grank, gwsize, rho, nu, p_max,p_min)
        else:
            loss_acc = train(distrib_model, device, train_loader, optimizer, epoch, grank, gwsize, rho, nu, p_max, p_min)
        ...
        if grank == 0 and lrank == 0:
            ...
            print('Epoch: {:03d}, Loss: {:.5f}, Test MSE: {:.5f}'.
                format(epoch, loss_acc, acc_test))
